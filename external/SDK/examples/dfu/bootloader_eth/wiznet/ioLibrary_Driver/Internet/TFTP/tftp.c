...
static int recv_udp_packet(int socket, uint8_t *packet, uint32_t len, uint32_t *ip, uint16_t *port)
{
	...
	if(sck_state == SOCK_UDP) {
		...
		if(recv_len) {
            NRF_LOG_INFO("First byte recv udp pre: 0x%02x\r\n", packet[4]);
			recv_len = recvfrom(socket, packet, len, (uint8_t *)ip, port);
            NRF_LOG_INFO("First byte recv udp: 0x%02x\r\n", packet[4]);
			if(recv_len < 0) {
...
static void recv_tftp_data(uint8_t *msg, uint32_t msg_len)
{
    ...
    NRF_LOG_INFO("First byte: 0x%02x\r\n", msg[4]);
	...
	switch(get_tftp_state())
	{
        ...
        if (nrf_dfu_flash_erase((uint32_t)MAIN_APPLICATION_START_ADDR, CEIL_DIV(BOOTLOADER_START_ADDR - MAIN_APPLICATION_START_ADDR, CODE_PAGE_SIZE), NULL) != NRF_SUCCESS)
        {
          ...
          NRF_LOG_INFO("Erase operation succesful\r\n");
        ...
        {
            nrf_delay_ms(10);
            if ((nrf_dfu_flash_store((uint32_t)(MAIN_APPLICATION_START_ADDR+(((data->block_num-1)/8) *  CODE_PAGE_SIZE)), (uint32_t *)write_data, CEIL_DIV(CODE_PAGE_SIZE, sizeof(uint32_t)), NULL)) != NRF_SUCCESS)
            {
                    NRF_LOG_INFO("WRITE ERROR\r\n");
            }
            NRF_LOG_INFO("First byte recv: 0x%02x\t addr: 0x%08x \t First byte stored: 0x%02x\r\n", *(g_socket_rcv_buf+4), g_socket_rcv_buf+4, write_data[((data->block_num-1) % 8) * TFTP_BLK_SIZE]);
            NRF_LOG_INFO("Store operation succesful\r\n");
...
int TFTP_run(void)
{
  ...
  int16_t len;
	...
	if(g_resend_flag) {
		if(tftp_time_cnt >= g_timeout) {
			...
			case STATE_RRQ:
                ...
                NRF_LOG_INFO("Option address: 0x%08x", &default_tftp_opt);
				break;
    ...
    NRF_LOG_INFO("First byte recv: 0x%02x", g_tftp_rcv_buf[4]);
